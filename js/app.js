...
function CookieCity(name, min, max, avg) {
  ...
  this.randomCustomer = function () {
    return Math.floor(Math.random() * (this.maxCookiesPerHour - this.minCookiesPerHour + 1) + this.minCookiesPerHour);
  };

  this.totalCookiesPerHour = function () {
    for (let i = 0; i < storeHours.length; i++) {
      let customersPerHour = this.randomCustomer();
      this.cookiesPerHour[i] = Math.ceil(customersPerHour * this.avgCookiesPerCustomer);
...
for (let i = 0; i < storeHours.length; i++) {
  ...
  hourlyTotals.textContent = seattle.cookiesPerHour[i] + tokyo.cookiesPerHour[i] + dubai.cookiesPerHour[i] + paris.cookiesPerHour[i] + lima.cookiesPerHour[i];
  newTableRow.appendChild(hourlyTotals);
}

let grandTotals = document.createElement('td');
grandTotals.textContent = seattle.dailyTotal + tokyo.dailyTotal + dubai.dailyTotal + paris.dailyTotal + lima.dailyTotal;
